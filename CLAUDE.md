...
- `components/ui/theme.ts`, `icons.tsx`, `brand.tsx` are the shared design tokens ("the dossier": graphite `#0e0e10` canvas, FLAT `bg-panel` surfaces with hairline borders — depth from rules and spacing, never glows/gradients; brass (`bg-brass`/`text-brass-soft`) = the one accent (primary action, active markers, section numerals); emerald/red = win/loss only, as plain colored text, not pills; amber only for warnings; Source Serif 4 display over Plex Sans/Mono) — reuse them rather than inventing new visual patterns.
