...
This product has a hard, deliberate scope: **no auth, no user accounts, no database or SERVER-side persistence of any kind, no saved history, no billing, no team workspaces, no analytics dashboards, no external AI calls, no scraping, no background jobs.** The one approved browser-persistence exception is the competitor watchlist (localStorage, `modules/competitor/watchlist.ts`): it may hold ONLY user-entered competitor info and fetched public-page signal summaries — never CSV data, memos, reports, or tokens. Do not add other localStorage keys. The whole flow is data in → KPI + context → deterministic analysis → one-page memo on the same page — nothing else is reachable, and nothing should become reachable as a "small addition." If a future version needs persistence, that is a new, explicit milestone to be requested — not something to add quietly while doing something else.
